...
export class QdrantService implements OnModuleInit {
  ...
  async search(
    ...
    return await this.qdrantClient.search(
      collectionName,
  ...
  /**
   * 특정 피그마 문서의 모든 벡터 포인트 삭제
   * @param collectionName 컬렉션 이름
   * @param figmaDocumentId 삭제할 피그마 문서 ID
   */
  async deleteFigmaDocumentPoints(
    collectionName: string,
    figmaDocumentId: string,
  ): Promise<number> {
    try {
      // 해당 figmaDocumentId를 가진 모든 포인트를 찾기
      const scrollResult = await this.qdrantClient.scroll(collectionName, {
        filter: {
          must: [
            {
      ...
      console.error(`Error deleting Figma document points: ${error}`);
      throw error;
    }
  }
}
