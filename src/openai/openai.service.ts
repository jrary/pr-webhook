...
6. 문서에 없는 정보는 추측하지 말고 "문서에 명시되지 않음"으로 표시하세요.`;
  ...
  /**
   * 피그마 화면 정보를 기반으로 질문에 답변 생성
   */
  async generateFigmaAnswer(
    question: string,
    screens: Array<{
      id: string;
      name: string;
    ...
    try {
      // 화면 컨텍스트를 구조화된 형식으로 변환
      const contextText = screens
        .map((screen, index) => {
          return `[화면 ${index + 1}: ${screen.name}]
타입: ${screen.type}
페이지: ${screen.pageName || '알 수 없음'}
링크: ${screen.link}
`;
        })
        .join('\n---\n\n');

      const systemPrompt = `당신은 피그마 디자인 파일을 기반으로 사용자에게 정확하고 실용적인 디자인 가이드를 제공하는 전문가입니다.

**핵심 원칙:**
1. **정확성**: 제공된 피그마 화면 정보만을 사용하며, 없는 내용은 추측하지 않습니다.
2. **명확성**: 복잡한 디자인 개념도 쉽게 이해할 수 있도록 설명합니다.
3. **완전성**: 화면 위치, 링크, 페이지 정보를 모두 포함합니다.

**답변 구조:**
1. **질문 분석**: 사용자가 찾고자 하는 화면이나 기능을 파악
2. **화면 추천**: 검색된 화면 중 어떤 것이 적합한지 설명
3. **화면 정보**: 각 화면의 이름, 타입, 위치, 링크 제공
4. **사용 가이드**: 필요시 화면을 어떻게 활용할지 안내

**화면 정보 표시 형식:**
- 화면 이름을 명확히 표시
- 타입 (FRAME, COMPONENT, INSTANCE) 표시
- 페이지 위치 표시
- 직접 링크 제공`;

      const userPrompt = `다음 피그마 화면들을 참고하여 질문에 답변해주세요:

${contextText}

질문: ${question}

위 화면 정보만을 사용하여, 사용자의 의도에 맞게 간결하고 명확하게 답변하세요.
- 화면 이름, 타입, 위치, 링크를 명확히 제공하세요.
- 여러 화면이 검색된 경우, 각 화면의 특징을 설명하세요.`;
      ...
      this.logger.error(
        `Failed to generate figma answer: ${(error as Error).message}`,
      );
      throw error;
    }
  }
  ...
  async chat(
    ...
    try {
      ...
      this.logger.error(`Failed to chat: ${(error as Error).message}`);
      throw error;
    }
  }
}
